...
class MedianFiltering
{
  public:
    ...
    Element& at( int index ) { return m_store[ index ]; }
    const Element& at( int index ) const { return m_store[ index ]; }
    const Element& top() const { return m_store[ prev( &m_index ) ]; }
    ...
    bool isTopAirvent() const
    {
        ...
        auto delta =
            abs( int32_t( at( t1Index ).m_deltaRelease
