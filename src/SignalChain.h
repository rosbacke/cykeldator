...
public:
	...
	struct Result {
		...
		uint32_t m_deltaAssert = 0; // Time duration (us) between 2 asserted pulses.
		uint32_t m_deltaRelease = 0; // Time duration (us) between 2 release pulses.
		uint32_t m_timeAsserted = 0; // Time duration (us) the last time point was asserted.
...
public:
	...
	struct Element {
		Element() = default;
		Element(uint32_t dt, uint32_t count) : m_deltaRelease(dt), m_count(count) {}
	...
	bool isTopAirvent() const
	{
		...
		auto delta = abs(int32_t(at(t1Index).m_deltaRelease + at(t2Index).m_deltaRelease - m_median));
