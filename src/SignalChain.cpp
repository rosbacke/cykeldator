 ...
 *
 * - delta time.
 * - prefiltering. (Time point are reasonable.)
...
void SignalChain::tickPoint(const TickPoint& tp) {
}
void SignalChain::sysTick(uint32_t sysTick) {
}

void RawSignalConditioning::reportTickPoint(const TickPoint& tp) {
	switch (m_state) {
	case State::NO_DATA:
		m_lastTick = tp;
	...
	case State::VALID:
		...
		m_result.m_deltaAssert = (tp.m_failingEdge - m_lastTick.m_failingEdge)
				/ 72;
		m_result.m_deltaRelease = (tp.m_raisingEdge - m_lastTick.m_raisingEdge)
				/ 72;
...
void RawSignalConditioning::reportSystick(uint32_t tick) {
	m_lastSystick = tick;
	if (m_state != State::NO_DATA) {
		if (tick - m_result.m_systick >= MAX_TP_AGE_MS) {
			m_state = State::NO_DATA;
		}
	}
}
